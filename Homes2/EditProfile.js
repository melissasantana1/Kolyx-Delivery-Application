...
export default function EditProfileT({ navigation, route }) {
  ...
  // --- TRANSPORTER OPTIONS (REINCLUÍDO) ---
  const transporterOptions = [
    'Car','Bicycle','Airplane','Motorcycle','Van','Public Transport',
    'Scooter','Boat','E-Scooter','Truck','On Foot','Tuk-Tuk','Private Jet',
    'Helicopter','Container'
  ];
  const [selectedTransportModes, setSelectedTransportModes] = useState(currentData?.transporterMode || []);
  const handleSelectTransportMode = (mode) =>
    selectedTransportModes.includes(mode)
      ? setSelectedTransportModes(selectedTransportModes.filter(m => m !== mode))
      : setSelectedTransportModes([...selectedTransportModes, mode]);
  ...
  const handleSave = () => {
    console.log("Dados salvos:", {
      ...
      transporterMode: selectedTransportModes
    });
    ...
    >
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 30 }}>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.countryContainer}>
            <View style={styles.flagCircle}>
              <CountryFlag isoCode={phoneCountry.cca2} size={18} />
            </View>
          ...
          {/* TRANSPORT MODE */}
          <Text style={[styles.label, { marginTop: 14 }]}>Transporter Mode</Text>
          <View style={styles.transporterGrid}>
            {transporterOptions.map((mode) => {
              const selected = selectedTransportModes.includes(mode);
              return (
                <TouchableOpacity
                  key={mode}
                  style={[
                    styles.transporterBox,
                    selected && styles.transporterBoxSelected,
                  ]}
                  onPress={() => handleSelectTransportMode(mode)}
                >
                  <Text
                    style={[
                      styles.transporterBoxText,
                      selected && { color: "#fff" },
                    ]}
                  >
                    {mode}
